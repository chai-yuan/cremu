...
static bool bus_read(void *context, u64 address, u8 size, u64 *data) {
    ...
    ERROR("address out of bounds, addr : %lx", address);
    return true;
}

static bool bus_write(void *context, u64 address, u8 size, u64 data) {
    struct BusDevice *bus = (struct BusDevice *)context;
